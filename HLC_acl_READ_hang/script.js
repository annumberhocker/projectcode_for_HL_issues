...
async function placeOrder(orderRequest) { // eslint-disable-line no-unused-vars
    //  Where is console?   console.log('placeOrder');
    ...
    order.orderer = factory.newRelationship(namespace, 'Seller', getParticipant().getIdentifier()); 
    //  orderRequest.orderer.getIdentifier());
...
async function updateOrderStatus(updateOrderRequest) { // eslint-disable-line no-unused-vars
    ...
    //  if orderStatus is OPENED, implies: Order received by Fulfiller; Fulfiller starts manufacturing process 

    if (updateOrderRequest.orderStatus === 'OPENED') {
        // create a shipment
        const shipment = factory.newResource(namespace, 'Shipment', 'Shipment-'+ updateOrderRequest.order.orderId);
        shipment.shipmentStatus = 'ACCEPTED';    
      
        // create shipmentDetails and assign to newly created shipment   
        shipment.shipmentDetails = factory.newConcept(namespace, 'ShipmentDetails', 'SD-'+updateOrderRequest.order.orderId);
      
        shipment.shipmentDetails.fulfillCo = 
          	factory.newRelationship(namespace, 'Fulfiller',
                                    updateOrderRequest.order.orderDetails.fulfillCo.getIdentifier());

        shipment.shipmentDetails.order = 
            factory.newRelationship(namespace, 'Order', updateOrderRequest.order.getIdentifier());

        shipment.shipmentDetails.notes = "Shipment was Created";
...
/**
 * Update the status of an Shipment
 * @param {org.acme.vehicle_network.UpdateShipmentStatus} updateShipmentStatus - the UpdateShipmentStatus transaction
 * @transaction
 */
async function updateShipmentStatus(updateShipmentRequest) { // eslint-disable-line no-unused-vars

    const namespace = 'org.acme.vehicle_network';

    //  if Target Status = SHIPPED; Done by Carrier or Fulfiller?
    if (updateShipmentRequest.ShipmentStatus === 'SHIPPED') {

        //  shipment.shipmentStatus = 'SHIPPED'; 
        updateShipmentRequest.shipment.shipmentDetails.notes = "Shipment is in transit.";

        //  update orderStatus to SHIPPED
        updateShipmentRequest.shipment.shipmentDetails.order.orderStatus = 'SHIPPED';
    } 
    //  if Target Status = RECEIVED; Done by Carrier
    else if (updateShipmentRequest.ShipmentStatus === 'RECEIVED') {

        //  shipment.shipmentStatus = 'RECEIVED'; 
        updateShipmentRequest.shipment.shipmentDetails.notes = "Shipment has been delivered.";

        //  update orderStatus to RECEIVED
        updateShipmentRequest.shipment.shipmentDetails.order.orderStatus = 'RECEIVED';
    } 

    // update the shipment 
    const shipment = updateShipmentRequest.shipment;
    shipment.shipmentStatus = updateShipmentRequest.shipmentStatus;
    const shipmentRegistry = await getAssetRegistry(namespace + '.Shipment');
    await shipmentRegistry.update(shipment);

    //  TODO:  check if the order is also updated with the status;
    //  else do another call to assetRegistry to update the order in 
    //  updateShipmentRequest.shipment.shipmentDetails.order
      
}

/**
 * Update the Seller notes
 * @param {org.acme.vehicle_network.UpdateOrder_SellerNotes} tx_UpdateOrder_SellerNotes - the UpdateShipmentStatus transaction
 * @transaction
 */
async function tx_UpdateOrder_SellerNotes(incomingData) { 
    ...
    order.orderDetails.sellerNotes     = incomingData.newNotes;  
    const assetRegistry = await getAssetRegistry(namespace + '.Order');
    await assetRegistry.update(order);
  }
  
/**
 * Update the Fulfiller notes
 * @param {org.acme.vehicle_network.UpdateOrder_FulfillerNotes} tx_UpdateOrder_FulfillerNotes - the UpdateShipmentStatus transaction
 * @transaction
 */
async function tx_UpdateOrder_FulfillerNotes(incomingData) { 
    ...
    // update the order
    const order = incomingData.order;
    order.orderDetails.fulfillerNotes  = incomingData.newNotes;
    const assetRegistry = await getAssetRegistry(namespace + '.Order');
    await assetRegistry.update(order);
  }
